<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yiic message' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE, this file must be saved in UTF-8 encoding.
 */
return array (
  '<strong>Latest</strong> updates' => '<strong>最后</strong> 更新',
  'Could not determine content container!' => '不确定内容的容器！',
  'Could not find content of addon!' => '找不插件内容！',
  'Could not find requested module!' => '找不到请求的模块！',
  'Insufficent permissions to create content!' => '无权创建内容！',
  'It looks like you may have taken the wrong turn.' => '访问出错了！！！',
  'Module is not on this content container enabled!' => '模块不能用于这个内容容器！',
  'Nothing found with your input.' => '没有找到相应内容。',
  'Oooops...' => '抱歉...',
  'Results' => '请求',
  'User Approvals' => '用户认证',
  'You cannot create public visible content!' => '你不能创造公共可见的内容！',
  'Your daily summary' => '你的每日概要',
  'Delete' => '删除',
  'Email settings' => '邮件设置',
  'Hide year in profile' => '隐藏年',
  'Account settings' => '帐户设置',
  'Administration' => '管理',
  'Back to dashboard' => '返回主面板',
  'Collapse' => '折叠',
  'Content Addon source must be instance of HActiveRecordContent or HActiveRecordContentAddon!' => '内容插件源必须是hactive record content或hactive record contentaddon实例',
  'Error' => '错误',
  'Expand' => '展开',
  'Invalid request.' => '无效的请求',
  'Keyword:' => '关键词',
  'Latest news' => '最新消息',
  'Logout' => '退出',
  'Menu' => '菜单',
  'My profile' => '我的资料',
  'New profile image' => '新图片',
  'Search' => '搜索',
  'Search for users and spaces' => '搜索用户或版块',
  'Show more results' => '展示更多结果',
  'Sorry, nothing found!' => '对不起，没有找到任何结果',
  'Space not found!' => '没找到版块',
  'User not found!' => '没找到用户',
  'Welcome to %appName%' => '欢迎来到%appName%',
);
